  ...
  "scripting logger" should {
      "work with a simple script" in {
        val scriptHandle = new ScriptHandle {
          ...
          override val script: String = readString(Paths.get("scripting/src/test/tweakflow/condition.tf"))
    ...
    "work with a MAC script" in {
      ...
      val signatureFile: Path = Paths.get("scripting/src/test/tweakflow/condition.tf.asc").toAbsolutePath
      ...
      val scriptHandle = new FileScriptHandle(scriptFile,
