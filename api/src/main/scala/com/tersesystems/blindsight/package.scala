      ...
      if (args.nonEmpty) {
        c.prefix.tree match {
          case Apply(_, List(Apply(_, partz))) =>
            ...
            if (t.isEmpty) {
              c.Expr(q"com.tersesystems.blindsight.Statement($format, com.tersesystems.blindsight.Arguments(..$arguments))")
            } else {
              // last throwable has the stack trace rendered.
              val throwable = t.last
              c.Expr(q"com.tersesystems.blindsight.Statement($format, com.tersesystems.blindsight.Arguments(..$arguments), $throwable)")
            }
          case _ =>
