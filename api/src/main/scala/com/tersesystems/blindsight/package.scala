...
package object blindsight {
  ...
  private object impl {
    ...
    private val OnlyOneMarker = "More than one marker argument is defined! Please provide a single Markers instance for multiple markers."
    private val MarkerAfterArguments = "Marker argument is after Arguments!  Please move the marker to the beginning of the string."
    private val MarkerAfterThrowable = "Marker argument is after throwable!  Please move the marker to the beginning of the string."
      ...
      if (args.nonEmpty) {
        c.prefix.tree match {
          case Apply(_, List(Apply(_, partz))) =>
            ...
            def assertMarkerConditions(): Unit = if (markersExpr.isDefined) {
            ...
            args.foreach { t =>
              ...
              el match {
                ...
                case _ if isThrowable(el) =>
                  ...
                  argumentList += c.Expr[Argument](q"com.tersesystems.blindsight.Argument($el.toString)")
                case _ =>
                  argumentList += c.Expr[Argument](q"implicitly[com.tersesystems.blindsight.ToArgument[${el.tpe}]].toArgument($el)")
              }
            }
            val arguments = q"com.tersesystems.blindsight.Arguments(..$argumentList)"
